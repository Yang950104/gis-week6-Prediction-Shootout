"""
梅雨事件空間預測分析 - 完整分析腳本
Part A: 空間預測對決 (Event 2: Plum Rain)

使用方式:
1. 在 Jupyter Notebook 中執行: %run plumrain_analysis.py
2. 或複製各個 CELL 區塊到 Notebook cells 中

備註：本腳本不包含 GeoTIFF 輸出（依作業規定擇一事件輸出即可）
"""
...
# 讀取 GeoJSON
gdf = gpd.read_file("../data/processed/plumrain_rainfall.geojson")
...
ax.set_title('Plum Rain - Variogram Analysis', fontsize=14, fontweight='bold')
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('../outputs/figures/plumrain_variogram_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.suptitle('Plum Rain - Spatial Interpolation Comparison (2x2)', 
             fontsize=14, fontweight='bold', y=0.98)
plt.savefig('../outputs/figures/plumrain_interpolation_2x2_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
ax.set_title('Plum Rain - Difference Map (Kriging - Random Forest)', 
             fontsize=14, fontweight='bold')
cbar = plt.colorbar(im, ax=ax)
cbar.set_label('Difference (mm)', rotation=270, labelpad=20, fontsize=12)
plt.tight_layout()
plt.savefig('../outputs/figures/plumrain_difference_kriging_rf.png', dpi=300, bbox_inches='tight')
plt.show()
...
ax.set_xlabel('Easting (m)', fontsize=12)
ax.set_ylabel('Northing (m)', fontsize=12)
ax.set_title('Plum Rain - Kriging Sigma Map (Uncertainty)', 
             fontsize=14, fontweight='bold')
ax.legend(loc='upper right')
cbar = plt.colorbar(im, ax=ax)
cbar.set_label('Sigma (mm)', rotation=270, labelpad=20, fontsize=12)
plt.tight_layout()
plt.savefig('../outputs/figures/plumrain_kriging_sigma_map.png', dpi=300, bbox_inches='tight')
plt.show()
...
# 合併所有 cell 資料
plumrain_results = {**cell1_data, **cell2_data, **cell3_data, **cell4_data, **cell5_data}

print("\n輸出圖表 (../outputs/figures/):")
print("  1. plumrain_variogram_comparison.png")
print("  2. plumrain_interpolation_2x2_comparison.png")
print("  3. plumrain_difference_kriging_rf.png")
print("  4. plumrain_kriging_sigma_map.png")

print(f"\n所有分析結果已儲存於 plumrain_results 字典中")
print(f"可用變數數量: {len(plumrain_results)}")
